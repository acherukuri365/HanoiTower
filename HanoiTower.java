...
public class HanoiTower {
	 ...
	 *	to be completed by student ...
	 */
	public void solve(int depth, int currPos, int endPos) {
		int temp = -1;
		if(currPos == 0 && end == 1) temp = 2;
		else if(currPos == 0 && endPos == 2) temp = 1;
		else if(currPos == 1 && endPos == 0) temp = 2;
		else if(currPos == 1 && endPos == 2) temp = 0;
		else if(currPos == 2 && endPos == 0) temp = 1;
		else if(currPos == 2 && endPos == 1) temp = 0;
		
		if(depth != 1)
			solve(depth - 1, currPos, temp);
			
		moveDisk(currPos, endPos);
		printTowers();
		
		if(depth != 1)
			solve(depth - 1, temp, endPos);
